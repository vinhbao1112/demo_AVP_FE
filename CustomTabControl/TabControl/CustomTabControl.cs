...
namespace System.Windows.Forms
{
    ...
    public class CustomTabControl : TabControl
    {
        ...
        public CustomTabControl()
        {
            ...
            this._useBorderStyle = true;
        }

        protected override void OnPaintBackground(PaintEventArgs pevent)
        {
            base.OnPaintBackground(pevent);
            if (UseTabPageBorderStyle)
            {
                PaintBorder(pevent);
            }
        }
        ...
        private void PaintBorder(PaintEventArgs e)
        {
            try
            {
                ...
                int width = e.ClipRectangle.Width - 1;
                int height = e.ClipRectangle.Height - 1;

                using (SolidBrush brush = new SolidBrush(Color.FromArgb(100, 145, 190)))
                {
                ...
                pen = new Pen(Color.FromArgb(108, 126, 143));
                e.Graphics.DrawRectangle(pen, new Rectangle(x, y, width, height));
                pen.Dispose();

                pen = new Pen(Color.FromArgb(113, 132, 150));
                e.Graphics.DrawRectangle(pen, new Rectangle(x + 1, y + 1, width - 2, height - 2));
                pen.Dispose();

                pen = new Pen(Color.FromArgb(119, 138, 157));
                e.Graphics.DrawRectangle(pen, new Rectangle(x + 2, y + 2, width - 4, height - 4));
                pen.Dispose();

                pen = new Pen(Color.FromArgb(83, 112, 142));
                e.Graphics.DrawRectangle(pen, new Rectangle(x + 3, y + 3, width - 6, height - 6));
                pen.Dispose();
            }
            catch { }
        }
        ...
        private Color _TabColor = Color.FromArgb(83, 125, 168);
        private Color _TabTextColor = Color.White;
        private Color _SelectedTabColor = SystemColors.Window;
        private Color _DisplayEnableTabTextColor = SystemColors.ControlText;
        private TabStyleProvider _StyleProvider;

        private List<TabPage> _TabPages;
        private bool _useBorderStyle;
        private bool _showCommunicationIcon = false;
        private Color _communicationConnectedColor = Color.LimeGreen;
        private Color _communicationDisconnectedColor = Color.Gray;
        private Color _communicationConnectingColor = Color.Yellow;
        private Color _communicationErrorColor = Color.Red;
        private Dictionary<TabPage, bool> _tabCommunicationVisibility = new Dictionary<TabPage, bool>();

        #endregion

        #region Public properties
        [Category("Appearance"), DefaultValue(typeof(Color), "LimeGreen"), RefreshProperties(RefreshProperties.All)]
        public Color CommunicationConnectedColor
        {
            get { return this._communicationConnectedColor; }
            set
            {
                if (this._communicationConnectedColor != value)
                {
                    this._communicationConnectedColor = value;
                    this.Invalidate();
                }
            }
        }

        [Category("Appearance"), DefaultValue(typeof(Color), "Red"), RefreshProperties(RefreshProperties.All)]
        public Color CommunicationDisconnectedColor
        {
            get { return this._communicationDisconnectedColor; }
            set
            {
                if (this._communicationDisconnectedColor != value)
                {
                    this._communicationDisconnectedColor = value;
                    this.Invalidate();
                }
            }
        }

        [Category("Appearance"), DefaultValue(typeof(Color), "Yellow"), RefreshProperties(RefreshProperties.All)]
        public Color CommunicationConnectingColor
        {
            get { return this._communicationConnectingColor; }
            set
            {
                if (this._communicationConnectingColor != value)
                {
                    this._communicationConnectingColor = value;
                    this.Invalidate();
                }
            }
        }
        ...
        [Category("Appearance"), DefaultValue(typeof(Color), "Yellow"), RefreshProperties(RefreshProperties.All)]
        public Color CommunicationErrorColor
        {
            get { return this._communicationErrorColor; }
            set
            {
                if (this._communicationErrorColor != value)
                {
                    this._communicationErrorColor = value;
                    this.Invalidate();
                }
            }
        }
        ...
        public bool ShowCommunicationIcon
        {
            get
            {
                return this._showCommunicationIcon;
            }
            set
            {
                if (this._showCommunicationIcon != value)
                {
                    this._showCommunicationIcon = value;
                    this.Invalidate();
                }
            }
        }
        ...
        public bool UseTabPageBorderStyle
        {
            get
            {
                return this._useBorderStyle;
            }
            set
            {
                this._useBorderStyle = value;
            }
        }
        ...
        [Category("Appearance"), DefaultValue(typeof(Color), "83, 125, 168"), RefreshProperties(RefreshProperties.All)]
        public Color DisplayTabColor
        {
            get { return this._TabColor; }
            set
            {
                if (this._TabColor != value)
                {
        ...
        [Category("Appearance"), DefaultValue(typeof(Color), "White"), RefreshProperties(RefreshProperties.All)]
        public Color DisplayTabTextColor
        {
            get { return this._TabTextColor; }
            set
            {
                if (this._TabTextColor != value)
                {
                    this._TabTextColor = value;
                    this.Invalidate();
                }
            }
        }

        [Category("Appearance"), DefaultValue(typeof(Color), "Window"), RefreshProperties(RefreshProperties.All)]
        public Color DisplaySelectedTabColor
        {
            get { return this._SelectedTabColor; }
            set
            {
        ...
        [Category("Appearance"), DefaultValue(typeof(Color), "ControlText"), RefreshProperties(RefreshProperties.All)]
        public Color DisplayEnableTabTextColor
        {
            get { return this._DisplayEnableTabTextColor; }
            set
            {
                if (this._DisplayEnableTabTextColor != value)
                {
                    this._DisplayEnableTabTextColor = value;
                    this.Invalidate();
                }
            }
        }
        ...
        public override Rectangle DisplayRectangle
        {
            get
            {
                ...
                else
                {
                    ...
                    Rectangle rect = new Rectangle(0, tabStripHeight, Width, Height - tabStripHeight);
                    switch (this.Alignment)
                    {
                        case TabAlignment.Top:
                            rect = new Rectangle(0, tabStripHeight, Width, Height - tabStripHeight);
                            break;
                        case TabAlignment.Bottom:
                            rect = new Rectangle(0, 0, Width, Height - tabStripHeight);
                            break;
                        case TabAlignment.Left:
                            rect = new Rectangle(tabStripHeight, 0, Width - tabStripHeight, Height);
                            break;
                        case TabAlignment.Right:
                            rect = new Rectangle(0, 0, Width - tabStripHeight, Height);
                            break;
                    }
                    return rect;
                }
        ...
        {
            this.BackupTabPages();
            return ((index >= 0) && (index < this.TabPages.Count));
        }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            else if (!this.DesignMode && index > -1 && this._StyleProvider.ShowTabCloser && this.GetTabCloserRect(index).Contains(e.Location))
            {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                Rectangle closerRect = this._StyleProvider.GetTabRect(this.ActiveIndex);
                if (this.RectangleToScreen(closerRect).Contains(Control.MousePosition))
                {
                    this.Invalidate();
                }
            }
        ...
        private void CustomPaint(ref Message m)
        {
            ...
            this.PaintTransparentBackground(this._BackBufferGraphics, this.ClientRectangle);
        ...
        private void DrawTabPage(int index, Graphics graphics)
        {
            ...
            using (GraphicsPath tabPageBorderPath = this.GetTabPageBorder(index))
            {
                ...
                if (this._Style != TabStyle.None)
                {
                    ...
                    //	Paint the tab
                    if (this._Style == TabStyle.SingleLoader)
                    {
                        this._StyleProvider.PaintTab(index, graphics, true);
                    }
                    else
                    {
                        this._StyleProvider.PaintTab(index, graphics, false);
                    }
                    ...
                    //	Draw the communication status icon
                    this.DrawTabCommunicationIcon(index, graphics);
        ...
        private void DrawTabBorder(GraphicsPath path, int index, Graphics graphics)
        {
            ...
            if (index == this.SelectedIndex)
            {
                borderPen = new Pen(this._StyleProvider.BorderColorSelected);
            }
            else
            {
                borderPen = new Pen(this._StyleProvider.BorderColor);
            }
        ...
        private void DrawTabText(int index, Graphics graphics)
        {
            ...
            if (index == 0 && this._StyleProvider.TabItemIndent > 0 && tabText.StartsWith(this._StyleProvider.TabItemIndentText))
            {
                tabText = tabText.Substring(this._StyleProvider.TabItemIndentText.Length);
            }

            if (this.SelectedIndex == index)
            {
                SolidBrush textBrush = new SolidBrush(this.DisplayEnableTabTextColor);
                graphics.DrawString(tabText, this.Font, textBrush, tabBounds, this.GetStringFormat());
                textBrush.Dispose();
            }
            else
            {
                if (this.TabPages[index].Enabled)
                {
                    SolidBrush textBrush = new SolidBrush(this.DisplayTabTextColor);
                    graphics.DrawString(tabText, this.Font, textBrush, tabBounds, this.GetStringFormat());
                    textBrush.Dispose();
                }
                else
                {
                    graphics.DrawString(tabText, this.Font, SystemBrushes.Window, tabBounds, this.GetStringFormat());
                }
            }
        }
        ...
        private void DrawTabCommunicationIcon(int index, Graphics graphics)
        {
            //	Draw a communication status indicator icon on the tab header only if enabled
            if (!this.ShowCommunicationIcon)
            {
                return;
            }

            // Don't draw icons if not needed
            TabPage page = this.TabPages[index];
            bool visibleForTab;
            if (!_tabCommunicationVisibility.TryGetValue(page, out visibleForTab))
            {
                visibleForTab = true;
            }
            ...
            Rectangle textRect = this.GetTabTextRect(index);
            Rectangle tabRect = this._StyleProvider.GetTabRect(index);
            int iconSize = 12;
            int offsetFromText = -iconSize - 12;
            ...
            switch (this.Alignment)
            {
                case TabAlignment.Top:
                case TabAlignment.Bottom:
                    //	Position icon beside text (to the right) with 10px offset, centered vertically
                    int textCenterY = tabRect.Y - 1 + (tabRect.Height - iconSize) / 2;
                    iconRect = new Rectangle(
                        textRect.Right + offsetFromText,
                        textCenterY,
                        iconSize,
                        iconSize
                    );
                ...
                case TabAlignment.Left:
                case TabAlignment.Right:
                    //	Position icon beside text (below) with 10px offset, centered horizontally
                    int textCenterX = tabRect.X + (tabRect.Width - iconSize) / 2;
                    iconRect = new Rectangle(
                        textCenterX,
                        textRect.Bottom + offsetFromText,
                        iconSize,
                        iconSize
                    );
                    break;
            ...
            //	Draw the communication icon as a colored circle
            //	Green = Connected, Red = Disconnected, Yellow = Connecting
            Color communicationColor = GetCommunicationStatusColor(index);

            //	Enable anti-aliasing for smooth circle
            graphics.SmoothingMode = SmoothingMode.AntiAlias;

            using (Brush statusBrush = new SolidBrush(communicationColor))
            {
                graphics.FillEllipse(statusBrush, iconRect);
            }

            //	Draw dark border around the circle
            using (Pen statusPen = new Pen(Color.Black, 2))
            {
                graphics.DrawEllipse(statusPen, iconRect);
            }
        ...
        private Color GetCommunicationStatusColor(int index)
        {
            //	This method returns the color based on communication status
            //	You can override this logic based on your needs
            ...
            //	Check if the tab has a Tag property indicating communication status
            //	Usage: tabPage.Tag = "Connected" | "Disconnected" | "Connecting"
            if (page.Tag != null)
            {
                string status = page.Tag.ToString().ToLower();
                ...
                switch (status)
                {
                    ...
                    case "offline":
                    case "0":
                        return this._communicationDisconnectedColor;
                    case "connected":
                    case "online":
                    case "1":
                        return this._communicationConnectedColor;

                    case "Error":
                    case "2":
                        return this.CommunicationErrorColor;

                    case "connecting":
                    case "busy":
                    case "3":
                        return this._communicationConnectingColor;
        ...
        public void SetTabCommunicationStatus(int index, string status)
        {
            //	Public method to set communication status for a tab
            //	status can be: "Connected", "Disconnected", "Connecting"
            
            if (this.IsValidTabIndex(index))
            {
                this.TabPages[index].Tag = status;
                this.Invalidate();
            }
        }

        public void SetTabCommunicationStatus(TabPage page, string status)
        {
            //	Overload to set communication status by TabPage reference
        ...
        private Rectangle GetTabTextRect(int index)
        {
            ...
            using (GraphicsPath path = this._StyleProvider.GetTabBorder(index))
            {
                ...
                if (this.Alignment <= TabAlignment.Bottom)
                {
                    while (!path.IsVisible(textRect.Right, textRect.Y))
                    {
                        textRect.Width -= 1;
                    }
                    while (!path.IsVisible(textRect.X, textRect.Y))
                    {
                        textRect.X += 1;
                        textRect.Width -= 1;
                    }
                ...
                {
                    while (!path.IsVisible(textRect.X, textRect.Bottom))
                    {
                        textRect.Height -= 1;
                    }
                    while (!path.IsVisible(textRect.X, textRect.Y))
        ...
        public Rectangle GetTabCloserRect(int index)
        {
            ...
            using (GraphicsPath path = this._StyleProvider.GetTabBorder(index))
            {
                ...
                if (this.Alignment <= TabAlignment.Bottom)
                {
                    ...
                    while (!path.IsVisible(closerRect.Right, closerRect.Y))
                    {
                        closerRect.X -= 1;
                    }
                    closerRect.X -= 4;
                ...
                else
                {
                    ...
                    while (!path.IsVisible(closerRect.X, closerRect.Bottom))
                    {
                        closerRect.Y -= 1;
                    }
                    closerRect.Y -= 4;
        ...
        public void SetTabCommunicationVisible(int index, bool visible)
        {
            if (this.IsValidTabIndex(index))
            {
                TabPage page = this.TabPages[index];
                _tabCommunicationVisibility[page] = visible;
                this.Invalidate();
            }
        }

        public void SetTabCommunicationVisible(TabPage page, bool visible)
        {
            if (page != null && this.TabPages.Contains(page))
            {
                _tabCommunicationVisibility[page] = visible;
        ...
        public bool GetTabCommunicationVisible(int index)
        {
            if (!this.IsValidTabIndex(index))
                return false;
            ...
            TabPage page = this.TabPages[index];
            bool visible;
            if (!_tabCommunicationVisibility.TryGetValue(page, out visible))
            {
                // Mặc định: true
                return true;
            }
        ...
        public bool this[TabPage page]
        {
            ...
            set
            {
                SetTabCommunicationVisible(page, value);
            }
        }
