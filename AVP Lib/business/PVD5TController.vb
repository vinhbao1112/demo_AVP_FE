...
#Region "Class Constants & Variables"
        ...
        Const DC_TARGET_MODE As String = "DC"
        ...
        Private m_objPVD5TChamber As PVD5TChamber = Nothing
#End Region
...
#Region "Constructors & Dispose"
        Public Sub New(ByVal EQName As String)
            ...
            m_tmrPullingTimer.Interval = ContainerData.GetPolling(ConstEnum.PVD5T_POLLING_CMD).Interval ''read from config file
            m_tmrPullingTimer.Enabled = True 'Start pulling
            m_RoutineExecutor = New ChamberRoutineExecutor(EQName)
            Me.EquipmentName = EQName
            m_objPVD5TChamber = DataManagerment.EquipmentManager.GetEquipment(Me.EquipmentName)
        End Sub
...
#Region "Helper Functions"
        ...
        Private Function IsAllInterlockOK() As Boolean
            ...
            Try
                Dim objChamber As PVD5TChamber = EquipmentManager.GetEquipment(Me.EquipmentName)
                ...
                If objChamber IsNot Nothing Then
                    blResult = (objChamber.Chamberinterlock_Devicenet_Comm = Equipment.WorkingStatuses.On) _
                                And (objChamber.Chamberinterlock_Lid_Closed_Status = Equipment.WorkingStatuses.On) _
                                And (objChamber.Chamberinterlock_PS_Interlock_Status = Equipment.WorkingStatuses.On) _
                                And (objChamber.Chamberinterlock_Target_Panels = Equipment.WorkingStatuses.On) _
                                And (objChamber.CG_Relay_Status = Equipment.WorkingStatuses.On) _
                                And (objChamber.Foreline_CG_Relay_Status = Equipment.WorkingStatuses.On)
                    If objSystemModule.Interlock_AirPressure_Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Air_Pressure_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.TargetVisible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Target1_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Target2Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Target2_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Target3Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Target3_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Target4Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Target4_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Target5Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Target5_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.ChamberInterlock_TurboWaterVisible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Turbo_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Bias_Matchbox_Water_Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Bias_MB_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Target_Matchbox_Water_Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Target_MB_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                    If objSystemModule.Interlock_SubstrateTableWater_Visible Then
                        blResult = blResult And (objChamber.Chamberinterlock_Substrate_Table_Water_Status = Equipment.WorkingStatuses.On)
                    End If
                End If
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
        ...
        Public Sub Pulling(ByVal source As Object, ByVal e As ElapsedEventArgs)
            ...
            Try
                m_tmrPullingTimer.Enabled = False
                ''Reconnect PM
                If (m_objPVD5TChamber IsNot Nothing) AndAlso m_objPVD5TChamber.ConnectionStatus <> Equipment.WorkingStatuses.On Then
                ...
                If m_IsInitOrReconnectPulling OrElse Not blHasSendWaferStatus Then
                    ...
                    blHasSendWaferStatus = DoSetWaferStatus(String.Format("{0:00}", CType(m_objPVD5TChamber.WaferStatus, Integer)))

                    If m_objPVD5TChamber.ListOfSlotWafer().Count > 0 Then
                        For Each mSlotWafer As Integer In m_objPVD5TChamber.ListOfSlotWafer()
                            PVD5TUtility.SetWaferSlotToPVD5T(Me.EquipmentName, mSlotWafer)
                        Next
                    End If

                    AVPLib.Log.avpLogger.Debug(Me.EquipmentName & " Reconnected, Send WaferStatus=" & m_objPVD5TChamber.WaferStatus.ToString)
                End If

                m_tmrPullingTimer.Enabled = True 'continue pulling

                If m_objPVD5TChamber.ControlStatus = DataManagerment.Equipment.ControlStatuses.ONLINE Then
                    ...
                    If ChamberModule IsNot Nothing Then
                        If ChamberModule.BiasPowerVisible AndAlso m_objPVD5TChamber.Bias_Communication_Status <> Equipment.WorkingStatuses.On Then
                            DoOffline()
                            Exit Sub
                        End If
                        If ChamberModule.RFTargetPowerVisible AndAlso m_objPVD5TChamber.Target_Mode = PVD5TChamber.TARGET_MODE_RF AndAlso m_objPVD5TChamber.RF_Target_Communication_Status <> Equipment.WorkingStatuses.On Then
                            DoOffline()
                            Exit Sub
                        ElseIf Not ChamberModule.RFTargetPowerVisible AndAlso ChamberModule.DCTargetPowerVisible AndAlso m_objPVD5TChamber.Target_Mode = PVD5TChamber.TARGET_MODE_DC AndAlso m_objPVD5TChamber.DC_Target_Communication_Status <> Equipment.WorkingStatuses.On Then
                            DoOffline()
                            Exit Sub
                        End If
                    End If
        ...
        Private Sub DoCheckRecipeTemplateVersion() Implements IRecipeProcessing.DoCheckRecipeTemplateVersion
            ...
            'Send Recipe Template Version
            PVD5TUtility.Check_Recipe_Template_Version(Me.EquipmentName, Utils.GetRecipeVersion(Me.EquipmentName))
        ...
        Private Sub DoSetOverride_Mode(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.OVERRIDE_MODE.ToString(), strData)
        ...
        '    Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
        '    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.MAINTENAINCE_MODE.ToString(), strData)
        ...
        Private Sub DoSetTargetX_Set_Kwh_Usage(ByVal index As Byte, ByVal val As String)
            ...
            Select Case index
                Case 1
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET1_SET_KWH_USAGE.ToString(), val)
                Case 2
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET2_SET_KWH_USAGE.ToString(), val)
                Case 3
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET3_SET_KWH_USAGE.ToString(), val)
                Case 4
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET4_SET_KWH_USAGE.ToString(), val)
            End Select
        ...
        Private Sub DoSetTarget_MagnatronX_On_Off(ByVal index As Byte, ByVal val As String)
            ...
            Select Case index
                Case 1
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET1_MAGNATRON_ON_OFF.ToString(), strData)
                Case 2
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET2_MAGNATRON_ON_OFF.ToString(), strData)
                Case 3
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET3_MAGNATRON_ON_OFF.ToString(), strData)
                Case 4
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET4_MAGNATRON_ON_OFF.ToString(), strData)
            End Select
        ...
        Private Sub DoSetTarget_Select_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET_SELECT_PROGRAM.ToString(), val)
            AVPLib.Log.coreLogger.Info("Leave DoSetTarget_Select_Program")
        End Sub
        Private Sub DoSetTargetX_Shutter_Status(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetTarget_Shutter_Status")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SELECTED_SHUTTER_PROGRAM.ToString(), val)
            AVPLib.Log.coreLogger.Info("Leave DoSetTarget_Shutter_Status")
        End Sub
        Private Sub DoSet_HomeShutter(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetTarget_Shutter_Status")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.HOME_SHUTTER_STATUS.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
            AVPLib.Log.coreLogger.Info("Leave DoSetTarget_Shutter_Status")
        End Sub
        Private Sub DoSetAuto_Pumpdown_Seq(ByVal val As String)
            ...
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                If val = STR_ON Then
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.AUTO_PUMPDOWN_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
                Else
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.AUTO_PUMPDOWN_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_CLOSED)
                End If
            Else
        ...
        Private Sub DoSetAuto_Vent_Seq(ByVal val As String)
            ...
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                If val = STR_ON Then
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.AUTO_VENT_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
                Else
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.AUTO_VENT_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_CLOSED)
                End If
            Else
                Me.ThrowAlarm("Failed to set Slit Valve Status for, " + Utils.chamberID2ChamberName(EquipmentName))
            End If
        ...
        Private Sub DoSetPump_Purge_Seq(ByVal val As String)
            ...
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                If val = STR_ON Then
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PUMP_PURGE_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
                Else
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PUMP_PURGE_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_CLOSED)
                End If
            Else
                Me.ThrowAlarm("Failed to set Slit Valve Status for, " + Utils.chamberID2ChamberName(EquipmentName))
            End If
        ...
        Private Sub DoSetIG_Degas_Seq(ByVal val As String)
            ...
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                If val = STR_ON Then
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.IG_DEGAS_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
                Else
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.IG_DEGAS_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_CLOSED)
                End If
            Else
                Me.ThrowAlarm("Failed to set Slit Valve Status for, " + Utils.chamberID2ChamberName(EquipmentName))
            End If
        ...
        Private Sub DoSetShutdown_Power_Seq()
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SHUTDOWN_POWER_SEQ.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        Private Sub DoRateOfRise()
            ...
            Dim IsPMIsoValveClose As Boolean = Utils.IsChamberSlitValveClose(Me.EquipmentName)
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                If PVD5TUtility.Start_Rate_Of_Rise(Me.EquipmentName,
        ...
        Private Sub DoStopRateOfRise()
            ...
            If Not PVD5TUtility.Stop_Rate_Of_Rise(Me.EquipmentName) Then
        ...
        Private Sub DoPumpdownCurve()
            ...
            Dim IsPMIsoValveClose As Boolean = Utils.IsChamberSlitValveClose(Me.EquipmentName)
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                If PVD5TUtility.Start_PumpDown_Curve(Me.EquipmentName,
                                            PDC_ROR_SampleTime.ToString(), PDC_ROR_WaitTime.ToString(), PDC_ROR_Description) Then
                    m_EventPumpDownAborted.Reset()
                Else
        ...
        Private Sub DoStopPumpdownCurve()
            ...
            If PVD5TUtility.Stop_PumpDown_Curve(Me.EquipmentName) Then
                m_EventPumpDownAborted.Set()
            Else
        ...
        Private Sub DoSetInitialize_Motion(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.INITIALIZE_MOTION.ToString(), strData)
        ...
        Private Sub DoSetAuto_Power_Seq(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.AUTO_POWER_SEQ.ToString(), strData)
        ...
        Private Sub DoSetChamberInterlock_PSRelay(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CHAMBERINTERLOCK_PS_INTERLOCK_STATUS.ToString(), strData)
        ...
        Private Sub DoSetGasX_Shutoff_Valve(ByVal iGasIndex As Byte, ByVal val As String)
            ...
            Select Case iGasIndex
                Case 1
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS1_SHUTOFF_VALVE.ToString(), strData)
                Case 2
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS2_SHUTOFF_VALVE.ToString(), strData)
                Case 3
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS3_SHUTOFF_VALVE.ToString(), strData)
                Case 4
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS4_SHUTOFF_VALVE.ToString(), strData)
                Case 5
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS5_SHUTOFF_VALVE.ToString(), strData)
            End Select
        ...
        Private Sub DoSetGasX_Flowrate_Program(ByVal index As Byte, ByVal val As String)
            ...
            Select Case index
                Case 1
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS1_FLOWRATE_PROGRAM.ToString(), val)
                Case 2
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS2_FLOWRATE_PROGRAM.ToString(), val)
                Case 3
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS3_FLOWRATE_PROGRAM.ToString(), val)
                Case 4
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS4_FLOWRATE_PROGRAM.ToString(), val)
                Case 5
                    PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.GAS5_FLOWRATE_PROGRAM.ToString(), val)
            End Select
        ...
        Private Sub DoSetMain_Dist_Valve(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.MAIN_DIST_VALVE.ToString(), strData)
        ...
        Private Sub DoSetSec_Dist_Valve(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SEC_DIST_VALVE.ToString(), strData)
            AVPLib.Log.coreLogger.Info("Leave DoSetSec_Dist_Valve")
        End Sub
        Public Sub DoSetRough_Valve_Status(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetRough_Valve_Status")
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.ROUGH_VALVE_STATUS.ToString(), strData)
            AVPLib.Log.coreLogger.Info("Leave DoSetRough_Valve_Status")
        End Sub
        Public Sub DoSetRough_Pump_In_Use(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetRough_Pump_In_Use")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.ROUGH_PUMP_IN_USE.ToString(), val)
            AVPLib.Log.coreLogger.Info("Leave DoSetRough_Pump_In_Use")
        End Sub
        Private Sub DoSetVent_Valve_Status(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetVent_Valve_Status")
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.VENT_VALVE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetIsolation_Valve_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.ISOLATION_VALVE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetHivac_Valve_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.HIVAC_VALVE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetForeline_Valve_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FORELINE_VALVE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetBaratron_Valve_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BARATRON_VALVE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetIG_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.IG_STATUS.ToString(), strData)
        ...
        Private Sub DoSetMechanical_Pump_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.MECHANICAL_PUMP_STATUS.ToString(), strData)
            AVPLib.Log.coreLogger.Info("Leave DoSetMechanical_Pump_Status")
        End Sub
        Public Sub DoSetSubstrate_Goto_Slot(ByVal val As String)
            PVD5TUtility.DoSetSubstrate_Goto_Slot(val, Me.EquipmentName)
        End Sub
        Public Sub DoSetSubstrate_Table_Up_Down_Status(ByVal val As String)
            PVD5TUtility.DoSetSubstrate_Table_Up_Down_Status(val, Me.EquipmentName)
        End Sub
        Public Sub DoSetSubstrate_Lift_Up_Down_Status(ByVal val As String)
            PVD5TUtility.DoSetSubstrate_Lift_Up_Down_Status(val, Me.EquipmentName)
        End Sub
        Private Sub DoSetSubstrate_Table_Lift_Home(ByVal val As String)
            PVD5TUtility.DoSetSubstrate_Table_Lift_Home(val, Me.EquipmentName)
        End Sub
        Private Sub DoSetSubstrate_Table_Rotate_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SUBSTRATE_TABLE_ROTATE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetSubstrate_Table_Rotate_Speed(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetSubstrate_Table_Rotate_Status")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SUBSTRATE_TABLE_ROTATE_SPEED.ToString(), val)
            AVPLib.Log.coreLogger.Info("Leave DoSetSubstrate_Table_Rotate_Status")
        End Sub
        Private Sub DoSetSubstrate_Table_Current_Position(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SUBSTRATE_TABLE_CURRENT_POSITION_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetWater_Pump_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_STATUS.ToString(), strData)
        ...
        Private Sub DoSetWater_Pump_Regen_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_REGEN_STATUS.ToString(), strData)
        ...
        Private Sub DoSetTurbo_Pump_On_Off(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TURBO_PUMP_ON_OFF.ToString(), strData)
        ...
        Private Sub DoSetVat_Valve_Controller_Pressure_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.VAT_VALVE_CONTROLLER_PRESSURE_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetVat_Valve_Percentage_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.VAT_VALVE_PERCENTAGE_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetVat_Valve_Controller_Auto_Zero(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.VAT_VALVE_CONTROLLER_AUTO_ZERO.ToString(), strData)
        ...
        Private Sub DoSetVat_Valve_Controller_Teach(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.VAT_VALVE_CONTROLLER_TEACH.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        Private Sub DoSetVat_Valve_Controller_Sizeadjust(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.VAT_VALVE_CONTROLLER_SIZEADJUST.ToString(), strData)
        ...
        Private Sub DoSetRF_Target_Power_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.RF_TARGET_POWER_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetRF_Target_MB_C1_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.RF_TARGET_MB_C1_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetRF_Target_MB_C2_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.RF_TARGET_MB_C2_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetRF_Target_MB_Match_Mode_Program(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.RF_TARGET_MB_MATCH_MODE_PROGRAM.ToString(), strData)
        ...
        Private Sub DoSetRF_Target_MB_Preset_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.RF_TARGET_MB_PRESET_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetBias_Power_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_POWER_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetBias_MB_Voltage_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_MB_VOLTAGE_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetBias_Power_Contact_On_Off(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_POWER_CONTACT_ON_OFF_STATUS.ToString(), strData)
        ...
        Private Sub DoSetBias_MB_C1_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_MB_C1_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetBias_MB_C2_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_MB_C2_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetBias_MB_Match_Mode_Program(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_MB_MATCH_MODE_PROGRAM.ToString(), strData)
        ...
        Private Sub DoSetDC_Target_Power_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.DC_TARGET_POWER_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetBias_MB_Preset_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.BIAS_MB_PRESET_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetDC_Target_Pluse_Mode_Status(ByVal val As String)
            ...
            Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.DC_TARGET_PULSE_MODE_STATUS.ToString(), strData)
        ...
        Private Sub DoSetDC_Target_Pulse_Frequency_Program(ByVal val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.DC_TARGET_PULSE_FREQUENCY_PROGRAM.ToString(), val)
        ...
        Private Sub DoSetDC_Target_Pulse_Width_Program(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetBias_MB_Preset_Program")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.DC_TARGET_PULSE_WIDTH_PROGRAM.ToString(), val)
            AVPLib.Log.coreLogger.Info("Leave DoSetBias_MB_Preset_Program")
        End Sub
        Private Sub DoSetDC_Target_Ramp_Time_Program(ByVal val As String)
            AVPLib.Log.coreLogger.Info("Enter DoSetBias_MB_Preset_Program")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.DC_TARGET_RAMP_TIME_PROGRAM.ToString(), val)
            AVPLib.Log.coreLogger.Info("Leave DoSetBias_MB_Preset_Program")
        End Sub
            ...
            Select Case ctrlName
                Case "Auto"
                    If PnlName = "BiasPowerSupply" Then
                        PVD5TUtility.Bias_Target_Auto(Me.EquipmentName, value)
                    Else
                        PVD5TUtility.RF_Target_Auto(Me.EquipmentName, value)
                    End If
                Case "Recall"
                    If PnlName = "BiasPowerSupply" Then
                        PVD5TUtility.Bias_Target_Recall(Me.EquipmentName, value)
                    Else
                        PVD5TUtility.RF_Target_Recall(Me.EquipmentName, value)
                    End If
                Case "Store"
                    If PnlName = "BiasPowerSupply" Then
                        PVD5TUtility.Bias_Target_Store(Me.EquipmentName, value)
                    Else
                        PVD5TUtility.RF_Target_Store(Me.EquipmentName, value)
                    End If
            End Select
        ...
        Private Sub SetHeaterZone1_SetPoint(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.HEATER_ZONE1_PROGRAM.ToString(), value)
        ...
        Private Sub SetHeaterZone2_SetPoint(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.HEATER_ZONE2_PROGRAM.ToString(), value)
        ...
        Private Sub SetHeaterZone1_OnOff(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.HEATER_ZONE1_ONOFF_PROGRAM.ToString(), strData)
        ...
        Private Sub SetHeaterZone2_OnOff(ByVal value As String)
            ...
            Dim strData As String = IIf(value = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.HEATER_ZONE2_ONOFF_PROGRAM.ToString(), strData)
        ...
        Private Sub SetMeasure_OnOff(ByVal value As String)
            ...
            Dim strData As String = IIf(value = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FILMETRIC_MEASURE.ToString(), strData)
        ...
        Private Sub SetGotoThickness_OnOff(ByVal value As String)
            ...
            Dim strData As String = IIf(value = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FILMETRIC_GOTO_THICKNESS.ToString(), strData)
        ...
        Private Sub SetGotoBaseLine_OnOff(ByVal value As String)
            ...
            Dim strData As String = IIf(value = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FILMETRIC_GOTO_BASELINE.ToString(), strData)
        ...
        Private Sub SetMeasure_SetPoint(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FILMETRIC_THICKNESS.ToString(), value)
        ...
        Private Sub SetFilmetric_Recipe_SP(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FILMETRIC_RECIPE_SP.ToString(), value)
        ...
        Private Sub SetIGFilament1(ByVal Val As String)
            ...
            Try
                PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SWITCH_IG_FILAMENT_PROGRAM.ToString(), 1)
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
            End Try
        ...
        Private Sub SetIGFilament2(ByVal Val As String)
            ...
            Try
                PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.SWITCH_IG_FILAMENT_PROGRAM.ToString(), 2)
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
            End Try
        ...
        Private Sub ClearAllAlarm(ByVal Val As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CLEAR_ALL_ALARM_PROGRAM.ToString(), 1)
        ...
        Private Sub DoSetIGIsolation_Valve(ByVal val As String)
            ...
            Try
                Dim strData As String = IIf(val = ConstEnum.STR_ON, ConfigurationValues.DEVICE_STATUS_OPEN, ConfigurationValues.DEVICE_STATUS_CLOSED)
                PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.IG_ISOLATION_VALVE_STATUS.ToString(), strData)
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
            End Try
        ...
        Public Overrides Sub DoTask(ByVal Message As String)
            ...
            Try
                ...
                Select Case strCtrlName
                    ...
                    Case "Target1Switch", "Target2Switch", "Target3Switch", "Target4Switch", "Target5Switch"    'Target_Select_Program
                    ...
                    Case "ACPower"
                        ...
                        '' Shutter X
                    Case "ShutterX"
                        DoSetTargetX_Shutter_Status(strVal)
                    Case "HomeShutter"
                        DoSet_HomeShutter(strVal)
                    Case "OverrideMode"
...
#Region "SetValueOfWaterPump"
        ...
        Private Sub SetCryoWP_Extended_PurgeTime(ByVal value As String)
            ...
            Try
                ...
                Else
                    SetCryo_Extended_PurgeTime(value)
                End If
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
            End Try
        ...
        Private Sub SetCryoWP_Pump_Restart_Delay(ByVal value As String)
            ...
            Try
                ...
                Else
                    SetCryo_Pump_Restart_Delay(value)
                End If
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
            End Try
        ...
        Private Sub SetCryoWP_Rough_To_Pressure(ByVal value As String)
            ...
            Try
                ...
                Else
                    SetCryo_Rough_To_Pressure(value)
                End If
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
            End Try
        ...
        ''' </author>
        ''' <summary>
        ''' Handles turning the cryo pump on or off based on the given string value.
        ''' </summary>
        Private Sub DoCryoOnOff(ByVal strval As String)
        ...
        ''' </author>
        ''' <summary>
        ''' Handles starting or stopping cryo regeneration based on the given string value.
        ''' </summary>
        Private Sub DoCryoRegen(ByVal strval As String)
            AVPLib.Log.coreLogger.Info("Enter DoTurnCryo")
            If strval = STR_ON Then
                DoCryoRegenOn()
            Else
        ...
        ''' </author>
        ''' <summary>
        ''' Handles fast regeneration on or off. On triggers DoFastRegen; off calls DoCryoRegenOff.
        ''' </summary>
        Private Sub DoCryoFastRegen(ByVal val As String)
        ...
        ''' </author>
        ''' <summary>
        ''' Initiates fast regen sequence: sets ISO valve status based on slit valve state,
        ''' then sends CRYO_REGEN command with STOPPED status to the PVD5T equipment.
        ''' </summary>
        Private Sub DoFastRegen()
            ...
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                ...
                If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_REGEN.ToString(), ConfigurationValues.DEVICE_STATUS_STOPPED) Then
                    m_EventPumpDownAborted.Reset()
                Else
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_REGEN command with CLOSED status to stop cryo regeneration.
        ''' </summary>
        Private Sub DoCryoRegenOff()
            AVPLib.Log.coreLogger.Info("Enter DoCryoRegenOff")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_REGEN.ToString(), ConfigurationValues.DEVICE_STATUS_CLOSED)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_POWER_ONOFF command with CLOSED status to power off the cryo pump.
        ''' </summary>
        Private Sub DoCryoOff()
            AVPLib.Log.coreLogger.Info("Enter DoCryoOff")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_POWER_ONOFF.ToString(), ConfigurationValues.DEVICE_STATUS_CLOSED)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_POWER_ONOFF command with OPEN status to power on the cryo pump.
        ''' </summary>
        Private Sub DoCryoOn()
            AVPLib.Log.coreLogger.Info("Enter DoCryoOn")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_POWER_ONOFF.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_REGEN command with OPEN status to start cryo regeneration.
        ''' </summary>
        Private Sub DoCryoRegenOn()
            AVPLib.Log.coreLogger.Info("Enter DoCryoRegenOn")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_REGEN.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_P_COMMANDS with extended purge time parameter and value to the equipment.
        ''' </summary>
        Private Sub SetCryo_Extended_PurgeTime(ByVal value As String)
            AVPLib.Log.coreLogger.Info("Enter SetCryo_Extended_PurgeTime")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_EXTENDED_PURGE_TIME & value)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_P_COMMANDS with pump restart delay parameter and value to the equipment.
        ''' </summary>
        Private Sub SetCryo_Pump_Restart_Delay(ByVal value As String)
            AVPLib.Log.coreLogger.Info("Enter SetCryo_Pump_Restart_Delay")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_PUMP_RESTART_DELAY & value)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_P_COMMANDS with rate-of-rise parameter and value to the equipment.
        ''' </summary>
        Private Sub SetCryo_Rate_Of_Rise(ByVal value As String)
            AVPLib.Log.coreLogger.Info("Enter SetCryo_Rate_Of_Rise")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_RATE_OF_RISE & value)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_P_COMMANDS with repurge cycles parameter and value to the equipment.
        ''' </summary>
        Private Sub SetCryo_Repurge_Cycles(ByVal value As String)
            AVPLib.Log.coreLogger.Info("Enter SetCryo_Repurge_Cycles")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_REPURGE_CYCLES & value)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_P_COMMANDS with rough-to-pressure parameter and value to the equipment.
        ''' </summary>
        Private Sub SetCryo_Rough_To_Pressure(ByVal value As String)
            AVPLib.Log.coreLogger.Info("Enter SetCryo_Rough_To_Pressure")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_ROUGH_TO_PRESSURE & value)
        ...
        ''' </author>
        ''' <summary>
        ''' Sends CRYO_P_COMMANDS with start-up temperature parameter and value to the equipment.
        ''' </summary>
        Private Sub SetCryo_Start_Up_Time(ByVal value As String)
            AVPLib.Log.coreLogger.Info("Enter SetCryo_Start_Up_Time")
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CRYO_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_START_UP_TEMPERATURE & value)
        ...
        Private Sub SetWaterPump_Extended_PurgeTime(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_EXTENDED_PURGE_TIME & value)
        ...
        Private Sub SetWaterPump_Pump_Restart_Delay(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_PUMP_RESTART_DELAY & value)
        ...
        Private Sub SetWaterPump_Rate_Of_Rise(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_RATE_OF_RISE & value)
        ...
        Private Sub SetWaterPump_Repurge_Cycles(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_REPURGE_CYCLES & value)
        ...
        Private Sub SetWaterPump_Rough_To_Pressure(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_ROUGH_TO_PRESSURE & value)
        ...
        Private Sub SetWaterPump_Start_Up_Time(ByVal value As String)
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.WATER_PUMP_P_COMMANDS.ToString(), ConstEnum.REGEN_PARAM_ID_START_UP_TEMPERATURE & value)
...
#End Region
        Public Sub SendRequestAllData() Implements IRecipeProcessing.SendRequestAllData
            ...
            If RobotConfigurationValues.SUPPORT_REQUEST_ALL_DATA_CHANGED Then
                PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.REQUEST_ALL_DATA.ToString, ConfigurationValues.DEVICE_STATUS_OPEN)
            End If
            ...
            If objTransferModule IsNot Nothing AndAlso objTransferModule.RoughPumpMachine1InUse IsNot Nothing Then
                objTransferModule.UpdateRoughPumpMachineInUseToChamber(objTransferModule.RoughPumpMachine1InUse)
            End If
            AVPLib.Log.coreLogger.Info("Enter SendCommandToPM")
        End Sub
        ...
        Public Sub SendResultOfCopyRecipeTemplate(ByVal blnResult As Boolean) Implements IRecipeProcessing.SendResultOfCopyRecipeTemplate
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(EquipmentName, PVD5TCommands.SYN_COPY_RECIPE_TEMPLATE.ToString(),
...
#Region "Process Recipe"
        Public Sub Process_Reset_Error() Implements IRecipeProcessing.Process_Reset_Error
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_CONTROL_DEVICE.ToString(), String.Format("{0:00}", valueSend))
        ...
        Public Function ResumeRecipe() As Boolean Implements IRecipeProcessing.ResumeRecipe
            ...
            Dim PVD5TChamber As Chamber = EquipmentManager.GetEquipment(Me.EquipmentName)
            PVD5TChamber.RunProcessResult = DataManagerment.Chamber.EnumRunProcessResult.Running
            PVD5TChamber.IsPauseInProcess = False
            Dim valueSend As Integer = ConstEnum.enumProcessStatus.eContinue
            Return PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_CONTROL_DEVICE.ToString(), String.Format("{0:00}", valueSend))
        ...
        Public Function SendToPM_CurrentAVPTime() As Boolean Implements IRecipeProcessing.SendToPM_CurrentAVPTime
            ...
            Return PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CURRENT_AVP_TIME.ToString(), DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss.fff"))
        ...
        Public Sub StartRecipe(ByVal val As String)
            ...
            Try
                ...
                Dim objPVD5TChamber As DataManagerment.PVD5TChamber = DataManagerment.EquipmentManager.GetEquipment(Me.EquipmentName)

                If (objPVD5TChamber IsNot Nothing AndAlso objPVD5TChamber.CurrentWaferCount >= objPVD5TChamber.WaferCapacity) Then
                    objPVD5TChamber.Recipe = val
        ...
        Function CopyRecipeFile(ByVal pathFrom As String, ByVal pathTo As String) As Boolean
            ...
            Try
                ...
                Else
                    ...
                    Utils.ThrowAlarm(String.Format(ContainerData.GetMessageText("EquipmentCouldNotCopyRecipe"),
            ...
            Try
                ...
                If bRet = False Then
                    If String.IsNullOrEmpty(errorMessage) Then
                        errorMessage = String.Format(ContainerData.GetMessageText("EquipmentCouldNotCopyRecipe"),
        ...
        Public Function StartSendDataRunFileName() As Boolean
            ...
            Else
                ...
                filenameWaferRun = GEMWaferID & "#" & strFolderName & "\" &
                   DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss") & ConstEnum.DataRunFileNameSeparator & LLName & "_" &
                ...
                If (Not Process_Control_Run_Data_File_Name(Me.EquipmentName, filenameWaferRun)) Then
                    Utils.ThrowAlarm(String.Format(ContainerData.GetMessageText("EquipmentCouldNotSendRunDatFile"),
        ...
        Public Shared Function Process_Control_Run_Data_File_Name(ByVal chamberName As String, ByVal data As String) As Boolean
            Return PVD5TUtility.SendCommandWithDataToPVD5T(chamberName, PVD5TCommands.PROCESS_CONTROL_SEND_RUN_DATA_FILE_NAME.ToString(), data)
        End Function
        ...
        Public Function StopRecipe() As Boolean Implements IRecipeProcessing.StopRecipe
            ...
            Dim bResult As Boolean = PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_CONTROL_DEVICE.ToString(), String.Format("{0:00}", valueSend))
            'Send reset error to clear command for start button to return to normal
            Thread.Sleep(1000)
            If bResult Then
                valueSend = ConstEnum.enumProcessStatus.eResetError
                bResult = PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_CONTROL_DEVICE.ToString(), String.Format("{0:00}", valueSend))
            End If
        ...
        Public Function PauseRecipe() As Boolean Implements IRecipeProcessing.PauseRecipe
            ...
            Dim PVD5TChamber As Chamber = EquipmentManager.GetEquipment(Me.EquipmentName)
            PVD5TChamber.IsPauseInProcess = True
        ...
        Public Function SetRecipeName(ByVal val As String) As Boolean Implements IRecipeProcessing.SetRecipeName
            ...
            Return PVD5TUtility.SendCommandWithDataToPVD5T(EquipmentName, PVD5TCommands.PROCESS_RECIPE_NAME.ToString(), val)
        End Function
        ...
        Public Function SetIsoValveStatus(ByVal strVal As String) As Boolean Implements IRecipeProcessing.SetIsoValveStatus
            Return PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName,
                                                    PVD5TCommands.ISOLATION_VALVE_STATUS.ToString(),
                                                    strVal)
        End Function
        ...
        Public Function StartProcessing() As Boolean Implements IRecipeProcessing.StartProcessing
            ...
            If SetIsoValveStatus(IIf(IsPMIsoValveClose, ConfigurationValues.DEVICE_STATUS_CLOSED, ConfigurationValues.DEVICE_STATUS_OPEN)) Then
                ...
                If (PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_CONTROL_DEVICE.ToString(), String.Format("{0:00}", valueSend))) Then
        ...
        Public Function StartProcessingCycleATM(ByVal val As String) As Boolean Implements IRecipeProcessing.StartProcessingCycleATM
            ...
            Dim bResult As Boolean = False
            If (PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_CYCLEATM_START.ToString(), val)) Then
                bResult = True
            End If
...
#Region "Online - Offline"
        Private Sub OnlineProc()
            ...
            Try
                ...
                If objChamber.ControlStatus = Equipment.ControlStatuses.MAINTENANCE Then
        ...
        Private Function CheckOnline() As Boolean
            ...
            Try
                ...
                If objChamber IsNot Nothing Then
                    objModule = ContainerData.GetRobotConfig(objChamber.Name)
                End If
                ...
                'If objModule.RFTargetPowerVisible AndAlso objChamber.Target_Mode = PVD5TChamber.TARGET_MODE_RF Then
                '    If (objChamber IsNot Nothing AndAlso objChamber.RF_Target_Communication_Status <> Equipment.WorkingStatuses.On) Then
                '        If m_blnThrowAlarm_When_Online Then
                ...
                If objModule.BiasPowerVisible Then
                    If (objChamber IsNot Nothing AndAlso objChamber.Bias_Communication_Status <> Equipment.WorkingStatuses.On) Then
                        If m_blnThrowAlarm_When_Online Then
                ...
                'If objModule.DCTargetPowerVisible AndAlso objChamber.Target_Mode = PVD5TChamber.TARGET_MODE_DC Then
                '    If (objChamber IsNot Nothing AndAlso objChamber.DC_Target_Communication_Status <> Equipment.WorkingStatuses.On) Then
                '        If m_blnThrowAlarm_When_Online Then
                ...
                'Buu Tran add 01/04/2012
                If Not (objChamber.IsPauseInProcess Or objChamber.IsProcessRunning) Then
                    If Not (objChamber.Hivac_Valve_Status = Equipment.WorkingStatuses.On) Then
                        If m_blnThrowAlarm_When_Online Then
                            Me.ThrowAlarm(Utils.chamberID2ChamberName(Me.EquipmentName) + ": Hivac valve is not opened" & GO_ONLINE_FAILED)
                        End If
                        AVPLib.Log.coreLogger.Info("Leave CheckOnline")
                        Return False
                    End If
                    If Not (objChamber.IGStatus = Equipment.WorkingStatuses.On) Then
                        If m_blnThrowAlarm_When_Online Then
                    ...
                    If objModule.TurboPumpVisible AndAlso (objChamber.Turbo_Pump_On_Off_Rb <> DataManagerment.Equipment.WorkingStatuses.On) Then
                        If m_blnThrowAlarm_When_Online Then
                ...
                If (objChamber.Is_MotionInitalized = Equipment.WorkingStatuses.Off) Then
                    If m_blnThrowAlarm_When_Online Then
...
#Region "Manula Action"
        ...
        Public Function DoSetWaferStatus(ByVal strStatus As String) As Boolean Implements IRecipeProcessing.DoSetWaferStatus
            ...
            Return PVD5TUtility.SetWaferStatusToCorona(Me.EquipmentName, strStatus)
        ...
        Public Sub DoResetShieldxKWH(ByVal TargetNumber As Integer, ByVal value As String)
            ...
            Try
                ...
                Select Case TargetNumber
                    Case 1
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET1_SET_SHIELD_QUART.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 1 Shield Quart value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 2
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET2_SET_SHIELD_QUART.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 2 Shield Quart value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 3
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET3_SET_SHIELD_QUART.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 3 Shield Quart value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 4
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET4_SET_SHIELD_QUART.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 4 Shield Quart value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 5
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET5_SET_SHIELD_QUART.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 4 Shield Quart value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                End Select
            Catch ex As Exception
...
#Region "Reset Target"
        Public Sub DoResetTargetKWHx(ByVal TargetNumber As Integer, ByVal value As String)
            ...
            Try
                ...
                Select Case TargetNumber
                    Case 1
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET1_SET_KWH_USAGE.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 1 KWH value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 2
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET2_SET_KWH_USAGE.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 2 KWH value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 3
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET3_SET_KWH_USAGE.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 3 KWH value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 4
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET4_SET_KWH_USAGE.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 4 KWH value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                    Case 5
                        If PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.TARGET5_SET_KWH_USAGE.ToString(), value) = False Then
                            Me.ThrowAlarm("Failed to set/reset Target 5 KWH value " + Utils.chamberID2ChamberName(EquipmentName))
                        End If
                End Select
            Catch ex As Exception
                AVPLib.Log.avpLogger.Error(ex.ToString())
...
#End Region
        ...
        ''' </author>
        ''' <summary>
        ''' Modified to read TargetMode directly from recipe XML instead of using ChamberConfig.
        ''' Determines RF/DC target type per step based on ProcessControl/TargetMode value.
        ''' </summary>
        Public Function GetAllTargetBaseOnRecipe(ByVal strRecipeName As String, ByVal strStationName As String) As System.Collections.Generic.List(Of String) Implements IRecipeProcessing.GetAllTargetBaseOnRecipe
            ...
            Try
                ...
                ' Count steps in recipe
                Dim iMaxStep As Integer = xmldoc.SelectSingleNode("Recipe/StepList").ChildNodes.Count

                For j As Integer = 0 To iMaxStep - 1
                    Dim stepNum As String = (j + 1).ToString()

                    ' Get TargetMode from ProcessControl
                    Dim targetModeNode As Xml.XmlNode = xmldoc.SelectSingleNode("Recipe/StepList/Step[SeqNo='" & stepNum & "']/ProcessControl/TargetMode")

                    If targetModeNode Is Nothing Then
                        Exit For
                    End If

                    ' Determine target type based on TargetMode
                    Dim strSelectTarget As String = "RFTargetPower"
                    Dim targetMode As String = targetModeNode.InnerText.Trim().ToUpper()

                    If targetMode = DC_TARGET_MODE Then
                        strSelectTarget = "DCTargetPower"
                    End If

                    ' Get target selection for this step
                    Dim XPATH_TARGET_SELECTOR As String = "Recipe/StepList/Step[SeqNo='" & stepNum & "']/" & strSelectTarget & "/TargetSelection"
                    Dim xmlNode As Xml.XmlNode = xmldoc.SelectSingleNode(XPATH_TARGET_SELECTOR)
        ...
        Public Function CheckingShieldsQuartzOverAlarmLimit(ByVal sTargetUsed As System.Collections.Generic.List(Of String)) As String Implements IRecipeProcessing.CheckingShieldsQuartzOverAlarmLimit
            ...
            Try
                Dim objPVD5TChamber As PVD5TChamber = DataManagerment.EquipmentManager.GetEquipment(Me.EquipmentName)
                ...
                If objConfigChamber IsNot Nothing AndAlso objConfigChamber.IsVisible AndAlso objConfigChamber.Type = SystemModule.ModuleType.PVD5T Then
                    ' Init variable
        ...
        Public Overridable Function SendProcessLotID() As Boolean Implements IRecipeProcessing.SendProcessLotID
            ...
            Try
                ...
                If ObjPM IsNot Nothing AndAlso ObjPM.GetWaferInfo() IsNot Nothing Then
                    Return PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_LOT_ID.ToString(), Utils.GetLotIDFromWafer(ObjPM.GetWaferInfo().WaferID))
                End If
            Catch ex As Exception
        ...
        Public Overridable Function SendProcessWaferID() As Boolean Implements IRecipeProcessing.SendProcessWaferID
            ...
            Try
                ...
                If ObjPM IsNot Nothing AndAlso Not String.IsNullOrEmpty(ObjPM.Process_Wafer_ID) Then
                    Return PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.PROCESS_WAFER_ID.ToString(), ObjPM.Process_Wafer_ID)
                End If
            Catch ex As Exception
                AVPLib.Log.coreLogger.Error(ex.ToString())
            End Try
...
#Region "Set VAT/VAC"
        Private Sub DoSetATMForelineCG()
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.FORELINE_CG_ATM.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        Private Sub DoSetATMRoughlineCG()
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.MECHANICAL_PUMP_CG_ATM.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        Private Sub DoSetATMPressureCG()
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CHAMBER_CG_ATM.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
        ...
        Private Sub DoSetVACPressureCG()
            ...
            PVD5TUtility.SendCommandWithDataToPVD5T(Me.EquipmentName, PVD5TCommands.CHAMBER_CG_VAC.ToString(), ConfigurationValues.DEVICE_STATUS_OPEN)
