...
Public Class PVD5TTargetControl
    Private m_defaultWidth As Integer = 340
...
#Region "Properties"
    ...
    ''' </summary>
    <DefaultValue(GetType(String), "T1")>
    ...
    ''' </summary>
    <DefaultValue(GetType(String), "T2")>
    ...
    ''' </summary>
    <DefaultValue(GetType(String), "T3")>
    ...
    ''' </summary>
    <DefaultValue(GetType(String), "T4")>
    ...
    <DefaultValue(GetType(String), "T5")>
    Public Property T5Text() As String
        Get
            Return m_t5text
        End Get
        Set(ByVal value As String)
            If m_t5text <> value Then
                m_t5text = value
                m_hasDataChanged = True
            End If
        End Set
    ...
    ''' </summary>
    <DefaultValue(GetType(TargetIndexs), "None")>
    Public Property ActiveTarget() As TargetIndexs
        Get
            Return m_currentTargetIndex
        End Get
        Set(ByVal value As TargetIndexs)
            If m_currentTargetIndex <> value AndAlso value >= TargetIndexs.None AndAlso value <= TargetIndexs.Target5 Then
    ...
    ''' </summary>
    <DefaultValue(GetType(DisplayStatus), "Off")>
            ...
            If m_currentTargetStatus <> value Then
                If value = DisplayStatus.Off OrElse value = DisplayStatus.On OrElse value = DisplayStatus.Unknow Then
                    ...
                    Select Case m_currentTargetIndex
                        ...
                        Case TargetIndexs.Target5
                            m_t5status = m_currentTargetStatus
                    End Select
...
#Region "Methods"
    ...
    Public Sub SetActiveTarget(ByVal index As Integer)
        ...
        Select Case index
            ...
            Case 4
                targetIndex = TargetIndexs.Target5
            Case Else
                isOther = True
        End Select
    ...
    Private Sub PaintImage()
        Try
            ...
            Using gr As Graphics = Graphics.FromImage(imgCtrl)
                If T5Installed Then
                    Dim p As Point = GetTargetLocation(5)
                    Dim img As Bitmap = GetTargetImage(m_t5status, T5Text)
                    gr.DrawImage(img, p)
    ...
    Private Function GetTargetImage(ByVal status As DisplayStatus, ByVal text As String) As Bitmap
        Try
            ...
            Dim offset As Double = 0.9F
            ' Resize imgTarget to smallest possible size
            imgTarget = New Bitmap(imgTarget, imgTarget.Width * offset, imgTarget.Height * offset)
            imgTargetPlasma = New Bitmap(imgTargetPlasma, imgTargetPlasma.Width * offset, imgTargetPlasma.Height * offset)
            imgTargetRamp = New Bitmap(imgTargetRamp, imgTargetRamp.Width * offset, imgTargetRamp.Height * offset)
