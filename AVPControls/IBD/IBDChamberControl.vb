...
Public Class IBDChamberControl
...
#Region "Methods"
    ...
    ''' <summary>
    ''' Generate IBD/VIBD chamber image.
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Overrides Function GenerateControlImage() As System.Drawing.Bitmap
        Try
            If ChamberType = AVPChamberTypes.VIBD Then
                Return GenerateVIBDImage()
            ElseIf ChamberType = AVPChamberTypes.PVD2T Then
                Return GeneratePVD2TImage()
            Else
                Return GenerateIBDImage()
            End If
    ...
    ''' <date>2016-06-23</date>
    ''' <summary>
    ''' Generate IBD Chamber image.
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GenerateIBDImage() As Bitmap
        Dim chamberImage As Bitmap = My.Resources.IBDVeeCo_Chamber_Empty
        Try
            Using g As Graphics = Graphics.FromImage(chamberImage)

                ' Draw heater panel.
                Dim frontPanelImage As Bitmap
                frontPanelImage = My.Resources.IBD_FrontPanel
                ...
                Select Case SlitValveStatus
                    Case AVPControls.AVPDataLib.DisplayStatus.Off
                        slitValveImage = My.Resources.IBD_Chamber_SlitValve_Closed
                    Case AVPControls.AVPDataLib.DisplayStatus.On
                        slitValveImage = My.Resources.IBD_Chamber_SlitValve_Open
                    Case Else
                        slitValveImage = My.Resources.IBD_Chamber_SlitValve_Unknown
                End Select
                ...
                If EtchSourceInstall Then
                    ...
                    Select Case EtchSourcePlasmaStatus
                        Case AVPControls.AVPDataLib.DisplayStatus.Unknow
                            etchSourceImage = My.Resources.IBD_EtchSource_Ramp
                        Case AVPControls.AVPDataLib.DisplayStatus.On
                            etchSourceImage = My.Resources.IBD_EtchSource_Plasma
                            If EtchShutterStatus = DisplayStatus.On Or ShutterOnFixtureInstall Or EtchShutterInstall = False Then
                                Dim etchFlowImage As Bitmap
                                etchFlowImage = My.Resources.IBD_EtchSource_PlasmaFlow
                                g.DrawImageUnscaled(etchFlowImage, 0, 0)
                                etchFlowImage.Dispose()
                            End If

                        Case Else
                            etchSourceImage = My.Resources.IBD_EtchSource_Normal
                    End Select
                ...
                If DepSourceInstall Then
                    ...
                    Select Case DepSourcePlasmaStatus
                        Case AVPControls.AVPDataLib.DisplayStatus.Unknow
                            depSourceImage = My.Resources.IBDVeeCo_DepoSource_Ramp
                        Case AVPControls.AVPDataLib.DisplayStatus.On
                            depSourceImage = My.Resources.IBDVeeCo_DepoSource_Plasma
                            If DepShutterStatus = DisplayStatus.On Or DepShutterInstall = False Then
                                Dim depFlowImage As Bitmap
                                depFlowImage = My.Resources.IBD_DepoSource_PlasmaFlow
                                g.DrawImageUnscaled(depFlowImage, 0, 0)
                                depFlowImage.Dispose()
                            End If

                        Case Else
                            depSourceImage = My.Resources.IBDVeeCo_DepoSource_Normal
                    End Select
                ...
                ' Draw chuck text.
                If Not String.IsNullOrEmpty(TargetMaterialText) Then
                    Dim f As New Font("Times New Roman", 11, FontStyle.Bold, GraphicsUnit.Point)

                    Using targetTextBrush As New SolidBrush(TargetMaterialTextColor)
                        If (TargetMaterialTextColor = Color.Red) Then
                            g.DrawString(TargetMaterialText, f, targetTextBrush, 191, 266)
                        Else
                            Dim strFormat As StringFormat = New StringFormat()
                            strFormat.Alignment = StringAlignment.Center
                            g.DrawString(TargetMaterialText, f, targetTextBrush, 220, 320, strFormat)
                        End If
                    End Using
    ...
    ''' <summary>
    ''' Generate VIBD chamber image.
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GenerateVIBDImage() As Bitmap
        Dim chamberImage As Bitmap = My.Resources.IBDVeeCo_Chamber_Empty
        Try
            Using g As Graphics = Graphics.FromImage(chamberImage)
                ...
                If Not String.IsNullOrEmpty(TargetMaterialText) Then
                    ...
                    Using targetTextBrush As New SolidBrush(TargetMaterialTextColor)
                        If (TargetMaterialTextColor = Color.Red) Then
                            g.DrawString(TargetMaterialText, f, targetTextBrush, 191, 266)
                        Else
                            g.DrawString(TargetMaterialText, f, targetTextBrush, 191, 320)
                        End If
                    End Using
